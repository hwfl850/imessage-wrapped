...
def probe_access() -> int:
    """Cheap Full Disk Access check for the Mac app's onboarding screen.

    macOS offers no API to ask whether we hold Full Disk Access, so the only honest
    test is to read the protected file and see what happens. Reads 16 bytes, copies
    nothing, and never opens SQLite — this runs on a timer while onboarding is up.

    0 = readable, 2 = permission denied, 3 = no Messages database on this Mac.
    """
    try:
        with open(CHAT_DB, "rb") as handle:
            handle.read(16)
    except PermissionError:
        return 2
...
# Handshake line the bundled Mac app reads off stdout to find the server's port.
PORT_BANNER = "WRAPPED_PORT"
...
def _exit_with_parent(httpd: http.server.ThreadingHTTPServer) -> None:
    """Shut down if the Mac app that launched us goes away.

    A clean quit terminates us directly, but a crash or force-quit would otherwise
    leave this server running with the user's messages loaded. Getting re-parented to
    launchd (PID 1) is the signal that our launcher is gone.
    """
    import os
    import time
...
def serve(
    port: "Optional[int]" = None, open_browser: bool = True, announce: bool = False
) -> int:
    if port is not None:
        try:
    ...
    if announce:
        # The Mac app parses this one line to learn where to point its web view. It has
        # to be flushed immediately — the app blocks on it before showing the report.
        print(f"{PORT_BANNER} {httpd.server_address[1]}", flush=True)
        threading.Thread(
            target=_exit_with_parent, args=(httpd,), name="parent-watch", daemon=True
        ).start()
    else:
...
def main(argv: "Optional[list[str]]" = None) -> int:
    ...
    parser.add_argument(
        "--announce-port",
        action="store_true",
        help=f"print '{PORT_BANNER} <port>' on startup, for the Mac app to parse",
    )
    parser.add_argument(
        "--probe-access",
        action="store_true",
        help="exit 0 if chat.db is readable, 2 if Full Disk Access is missing",
    )
